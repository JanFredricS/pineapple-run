...
| `ORIGINAL_EXPERT_LINE` (test/integration/driver.ts) | new: **9 m/s, then 6 m/s from x = 200** (**S8a: 9.3 m/s**) | A reference line that reaches the goal with 10/15 in 38.3 s, proving the course can be finished. A constant 8.5 or 9.5 m/s gets stuck. |
...
- **V-sag** (test/integration/vSag.test.ts): the playtest's articulated cart is rebuilt (two halves joined only by three shocks), loaded, and driven across the workbench washboard (37–56 m) at 2.5, 3, 4, 4.5, 4.8, 5, 5.2, 5.5 and 6 m/s.
  - With the S6T emulated stops the shocks ran to 1.36× rest, and at 5 m/s the cart stuck for good at **42.6 m**, which is the playtest's failure.
  - With the spring alone they reached 0.44× and the halves folded 36.5°.
  - With the real limits the shocks stay at **0.79–1.18×**, the fold is at most 11°, and every speed crosses.
  - With real limits but the old 0.9 friction, 3 m/s still stalls at 50.7 m, because the load brakes a wheel. Both changes are needed.
...
- **S1 timings are bit-identical**: full throttle 8.683 s, cruise 3 m/s 17.25 s, cruise 5 m/s 13.05 s. The spill scenario's goal time is unchanged at 11.917 s, but its counts moved from 11 lost / 4 delivered to **7 / 9**: the load slides off the wheels instead of being thrown by them.

...
/**
 * The reference cart a course's pace notes are written for. K1: Kitchen's
 * sink is too wide for the example cart (test/integration/kitchen.test.ts),
 * so its line is driven by the purpose-built Kitchen Bridger.
 */
const cartFor = (id: string): CartDesign => (id === 'kitchen' ? kitchenBridger() : exampleCart());
...
describe('the reference cart completes every premade level (pace-note scripted driver; example cart, the Kitchen Bridger on kitchen)', () => {
  for (const id of LEVELS) {
    it(`${id}: goalReached with pineapples delivered and a non-zero rating`, async () => {
      const s = await session(cartFor(id), id);
      try {
...
describe('S6T: pacing beats flooring it (every premade level)', () => {
  ...
  // hazards (beach: the dune jump; kitchen: the sink, K1: 6.5 m/s, 10 for the drainer,
  // with the Kitchen Bridger; workbench: the saw-horse jump). Holding → the
  // whole way must score clearly less.
  ...
  for (const id of LEVELS) {
    it(`${id}: the pace-note line out-scores holding → by >= 5 points`, async () => {
      const a = await session(cartFor(id), id);
      const b = await session(cartFor(id), id);
      try {
...
describe('S6T audit-1 #4: every premade level has a risk/reward shortcut (PLAN S6)', () => {
  ...
  // 4.62 s through the pool, rating 89 vs 85; workbench 2.02 vs 4.92 s, 88
  // vs 82 — all 15/15. K1: kitchen's reference cart is the 17.5 m Kitchen
  // Bridger, which never flies the pool (its wheels reach the floor at any
  // approach speed, 7.5-14 m/s measured): see the kitchen test below and RESIDUALS R30.
  const SAFE_SPEED = 5;
  interface Line {
    delivered: number;
    rating: number;
  ...
  const censusFinds = (id: (typeof LEVELS)[number]) => {
    const a = PREMADE[id]();
    const pool = a.features.find((f) => f.kind === 'shortcut')!;
    // the census finds exactly this shortcut in the geometry
    const fin = a.features.find((f) => f.kind === 'finish')!;
    // the whole playable stretch, to the goal line (K1 audit-2 #2); the pit edges are finish geometry, kept sharp
    const c = census(a.level, { x0: plateauRange(a.level.cartStart).maxX, x1: a.level.goal.lineX }, (x) => targetSpeed(a.pace, x), { keepSharp: [fin] });
  ...
  // K1: the Kitchen Bridger rolls through the drainer at pace instead of
  // flying it. Measured: pace line 3.90 s through the pool, 15/15 rating 75;
  // careful 5.60 s, 15/15 rating 63.
  it('kitchen: the pool is still found in the geometry; the bridger at pace rattles through it faster than the careful line, both keeping the load', async () => {
    const { a, careful } = censusFinds('kitchen');
    const paced = await drive('kitchen', a.pace);
    const safe = await drive('kitchen', careful);
    expect(paced.delivered).toBe(TOTAL_PINEAPPLES);
    expect(safe.delivered).toBe(TOTAL_PINEAPPLES);
    // both roll across the floor (the long cart never flies it)
    expect(paced.deepest).toBeGreaterThan(-2);
    expect(safe.deepest).toBeGreaterThan(-2);
    expect(safe.section - paced.section, `paced ${paced.section.toFixed(2)} s vs careful ${safe.section.toFixed(2)} s`).toBeGreaterThanOrEqual(1);
    expect(paced.rating - safe.rating, `paced ${paced.rating} vs careful ${safe.rating}`).toBeGreaterThanOrEqual(5);
  }, 60_000);
});

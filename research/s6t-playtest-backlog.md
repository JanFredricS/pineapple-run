...
| 4 | Shock-hung wheels fold under the bed | **Tuned** | 35c4bc2. Shocks are now 8 Hz / 0.7, with emulated 0.8–1.15× bump stops in compound.ts. The fold cannot happen inside these limits. Residual: the articulated cart, whose two halves are joined only by shocks, can still sag into a V and belly on a washboard tooth. This is chaotic; it was seen at workbench 42 m. Real joint limits need engine.ts. Audit-1 #2 (473b844, 155b65f): the stops also run on a DAMAGED cart, after a body is lost past the kill plane. Before this fix the controller's survivors-only branch skipped them. **S8a: Fixed.** The limits are now real Box2D distance-joint limits at the same 0.8–1.15×, and the emulation is removed. test/integration/vSag.test.ts runs the loaded articulated cart over the workbench washboard at 2.5–6 m/s: every speed crosses with shocks at 0.78–1.18× and ≥ 12/15 aboard. An executable control on the same rig (limits switched off, pineapple–wheel back at 0.9) runs the shocks to 1.25–1.47× and sticks at 46.5 m at 5 m/s. (A one-off scratch run of the S6T emulated stops stuck at 42.6 m at 5 m/s; that build is not kept as a test.) |
...
| 6 | The original course cannot be finished | **Tuned (presentation); geometry deferred to the user** | 17558bb. All 71 vertices are untouched. The course is labelled "Bonus · Expert" with an expert blurb. The reference expert line (9 m/s, then 6 m/s from x = 200) reached the goal with 10/15 in 38.3 s, and an acceptance test checks it. **S8a: the line is now 9.3 m/s, then 6 m/s from x = 200 (10/15, 37.9 s); 9 m/s now gets stuck at about 94 m** (see TUNING.md "S8a"). Audit-1 #5 (0535f68): the test now requires ≥ 9 delivered on that line and three nearby ones, with the reference line exactly 10; it used to accept ≥ 5. The course is a knife edge: in S6T a constant 8.5 or 9.5 m/s got stuck, and so did cruise speeds of 8.7, 8.9 and 9.1–9.3. After S8a, 9.3, 10.0 and 10.1 finish; 9.0–9.25, 9.9 and 10.05 get stuck. Any geometry compromise is the user's call. |
...
| 9 | Pineapples lock wheels | **Deferred in S6T → Fixed in S8a** | engine.ts gained `setPairFriction` (`b2World_SetFrictionCallback` keyed on `userMaterialId`). Pineapple–wheel friction is 0.3; every other pair is bit-identical. Pinch test: a driven wheel wedged by pineapples goes from |ω| < 0.5 to > 18 rad/s. Trade-off: wheels no longer claw up a pineapple heap (R22). |
